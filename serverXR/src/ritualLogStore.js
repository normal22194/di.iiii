...
const ACTIONS = new Set(['place', 'claim', 'revoke', 'advance', 'residue'])
...
const appendRitualLog = ({ spaceId, actorLabel = '', action, targetLabel = '', detail = {} }) => {
  ...
  getDb().prepare(
    `INSERT INTO ritual_log (id, space_id, actor_label, action, target_label, detail, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    spaceId,
